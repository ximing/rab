    ...
    return (
      <Card title="带生命周期的类组件">
        <Space direction="vertical" size="middle" style={{ width: "100%" }}>
          ...
          <Paragraph type="secondary">
            这个类组件有完整的生命周期方法。<code>view</code>{" "}
            会正确处理这些生命周期， 并在组件卸载时自动清理
            reaction。打开控制台查看生命周期日志。
          </Paragraph>
        </Space>
      </Card>
    );
  ...
  return (
    <Card title="函数组件（使用 view）">
      <Space direction="vertical" size="middle" style={{ width: "100%" }}>
        ...
        <Paragraph type="secondary">
          <code>view</code> 也可以用于函数组件，内部会自动使用{" "}
          <code>observer</code>。 推荐函数组件直接使用 <code>observer</code>
          ，类组件使用 <code>view</code>。
        </Paragraph>
      </Space>
    </Card>
  );
