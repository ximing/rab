...
export default function QuickStart() {
  return (
    <div>
      <h1>快速开始</h1>
      <p>从 0 到 1 跑通 RAB 的最小链路：安装 → 三段式写法 → 一个可交互的完整示例。</p>

      <h2>安装</h2>
      <CodeBlock language="bash">{installCode}</CodeBlock>
      <p>
        <code>@rabjs/react</code> 内部重新导出了 <code>@rabjs/service</code> 和{' '}
        <code>@rabjs/observer</code> 的全部 API，日常开发只从 <code>@rabjs/react</code> 一个入口
        import 即可。
      </p>
      ...
      <h3>1. 定义 Service</h3>
      <p>
        状态和业务方法都放进一个继承 <code>Service</code> 的类里。不需要写 reducer、action type
        或任何模板代码：
      </p>
      <CodeBlock language="ts" title="CounterService.ts">
        {step1Code}
      </CodeBlock>

      <h3>2. observer + useService</h3>
      <p>
        用 <code>observer</code> 包装组件（这样它才会追踪响应式依赖）， <code>useService</code>{' '}
        取出服务实例，然后像普通对象一样读写：
      </p>
      ...
      <p>
        服务实例由容器创建和管理。<code>bindServices</code> 在组件外面包一层容器， 并把 Service
        注册进去：
      </p>
      ...
      <p>
        基类为每个方法自动维护 <code>{'$model.<方法名> = { loading, error }'}</code>：异步方法调用时{' '}
        <code>loading</code> 立即变 <code>true</code>，结束后复位；reject 的错误会写入{' '}
        <code>error</code>。<code>$model</code> 本身也是响应式的，可以直接渲染。 完整例子见{' '}
        <Link to="/guides/demos">在线 Demo 的异步加载</Link>。
      </p>

      <h3>不写 bindServices 会怎样？</h3>
      <p>
        非严格模式下，<code>useService</code> 遇到没注册的服务会自动注册到全局容器，
        所以不写也能跑。但 <code>bindServices</code> 给你的是独立作用域： 同一个 Service
        类在两个页面各有一份实例、互不影响，组件卸载时容器也随之销毁。
      </p>

      <h3>需要手动配置批处理吗？</h3>
      <p>
        不需要。<code>@rabjs/react</code> 被 import 时会自动把全局 scheduler 配成 React 的{' '}
        <code>unstable_batchedUpdates</code>（见包的 batch 模块）。 只有脱离 React 单独用{' '}
        <code>@rabjs/observer</code> 时才可能要用 <code>configure()</code>。
      </p>
      ...
      <ul>
        ...
        <li>
          <Link to="/guides/service">Service 服务容器</Link>
          ：服务间依赖（this.resolve）、Container、生命周期
        </li>
        <li>
