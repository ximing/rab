  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>window.__RS_ROOT_CONTAINER__</code>，让你（或 AI、E2E 脚本）在 React
        组件树之外直接查看和操作 Service 实例。
      </p>

      <h2>接入</h2>
      <CodeBlock language="bash">{installCode}</CodeBlock>
      <p>
        只需要一行初始化。它与框架无关，SSR 环境会自动跳过；以全局容器为根， 所有{' '}
        <code>bindServices</code> 创建的子容器都在它的遍历范围内：
      </p>
      ...
      <p>
        <code>listServices()</code> 返回的是真实的内存对象，控制台里改属性、调方法， 页面上的{' '}
        <code>observer</code> 组件会照常响应——这也是它和「状态快照」类工具的区别。
      </p>
