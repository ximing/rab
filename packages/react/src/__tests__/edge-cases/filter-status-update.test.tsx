              ...
              {productService.filteredProducts.map((product) => {
                ...
                return (
                  <div key={product.id} data-testid={`product-${product.id}`}>
                    <h3>{product.name}</h3>
                    <span data-testid={`status-${product.id}`}>
                      {statusText}
                    </span>
                  </div>
                );
  ...
  describe("对比测试：两种方式的差异", () => {
    it("方式1（定义时包裹）vs 方式2（导出时包裹）", async () => {
      ...
      const ProductPage1 = observer(() => (
        <div>
          <div data-testid="count1">
            {productService1.filteredProducts.length}
          </div>
          <button
            onClick={() => productService1.setFilterStatus("active")}
            data-testid="btn1"
          >
            Filter
          </button>
      ...
      const ProductPage2Base = () => (
        <div>
          <div data-testid="count2">
            {productService2.filteredProducts.length}
          </div>
          <button
            onClick={() => productService2.setFilterStatus("active")}
            data-testid="btn2"
          >
            Filter
          </button>
