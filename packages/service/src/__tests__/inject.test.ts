...
describe("@Inject Decorator", () => {
  ...
  describe("依赖解析", () => {
    ...
    test("应该在依赖未找到时抛出错误", () => {
      ...
      const userService = testContainer.resolve<ServiceWithMissingDep>(
    ...
    test("应该在依赖所在容器被销毁时抛出错误", async () => {
      ...
      const userService =
        tempContainer.resolve<ServiceWithTempDep>(ServiceWithTempDep);
  ...
  describe("字符串标识符", () => {
    test("应该支持字符串标识符的注入", () => {
      ...
      const service =
        container.resolve<ServiceWithStringId>(ServiceWithStringId);
  ...
  describe("Symbol 标识符", () => {
    test("应该支持 Symbol 标识符的注入", () => {
      ...
      const service =
        container.resolve<ServiceWithSymbolId>(ServiceWithSymbolId);
      const repo = service.getRepo();
  ...
  describe("子容器继承", () => {
    ...
    test("子容器可以覆盖父容器的依赖", () => {
      ...
      const userService =
        childContainer.resolve<UserServiceWithRepo>(UserServiceWithRepo);
      const data = userService.getData();
  ...
  describe("在 getter 中访问注入的属性", () => {
    class OrderReceivingStoreService extends Service {
      options = [
    ...
    class TMSWorkbenchService extends Service {
      @Inject(OrderReceivingStoreService)
      private orderReceivingStoreService!: OrderReceivingStoreService;

      get poiOptions(): Array<{ value: number; label: string }> {
        // 这里访问 this.orderReceivingStoreService 应该能正常获取到实例
        return this.orderReceivingStoreService.options;
      }
    }

    test("应该能在 getter 中正常访问注入的属性", () => {
      container.register(OrderReceivingStoreService);
      container.register(TMSWorkbenchService);

      const workbenchService =
        container.resolve<TMSWorkbenchService>(TMSWorkbenchService);

      // 访问 getter，内部会访问注入的属性
      const options = workbenchService.poiOptions;

      expect(options).toBeDefined();
      expect(Array.isArray(options)).toBe(true);
      expect(options.length).toBe(2);
      expect(options[0]).toEqual({ value: 1, label: "Option 1" });
    });

    test("应该多次访问 getter 返回相同的注入实例", () => {
      container.register(OrderReceivingStoreService);
      container.register(TMSWorkbenchService);
      ...
      // 多次访问 getter
      const options1 = workbenchService.poiOptions;
      const options2 = workbenchService.poiOptions;
