...
const createTestContainers = (count: number): Container[] => {
  return Array.from({ length: count }, (_, i) =>
    createContainer(`test-container-${i}`)
  );
};
...
describe("Container - 专门为 Service 定制的 IoC 容器", () => {
  ...
  describe("树形容器结构", () => {
    ...
    test("子容器可以覆盖父容器的服务", () => {
      container.register(
        "config",
        () => new ConfigService({ source: "parent" })
      );
    ...
    test("应该支持多层嵌套", () => {
      ...
      expect(container.resolve<ConfigService>("level").getConfig().value).toBe(
        0
      );
  ...
  describe("错误处理", () => {
    ...
    test("工厂函数应该返回 Service 实例，否则抛出异常", () => {
      // 返回普通对象
      container.register("invalidService", (() => ({
        name: "invalid",
      })) as any);
      expect(() => {
  ...
  describe("更换父容器 (setParent)", () => {
    ...
    test("更换父容器后应该能继承新父容器的服务", () => {
      ...
      parent1.register(
        "service1",
        () => new ConfigService({ value: "from-parent1" })
      );
      parent2.register(
        "service2",
        () => new ConfigService({ value: "from-parent2" })
      );

      // 初始状态：child 继承 parent1 的服务
      expect(child.resolve<ConfigService>("service1").getConfig().value).toBe(
        "from-parent1"
      );
      ...
      child.setParent(parent2);
      expect(child.resolve<ConfigService>("service2").getConfig().value).toBe(
        "from-parent2"
      );
  ...
  describe("复杂场景", () => {
    ...
    test("应该支持复杂的树形结构", () => {
      ...
      // 验证继承关系
      expect(moduleA1.resolve<ConfigService>("appName").getConfig().name).toBe(
        "MyApp"
      );
