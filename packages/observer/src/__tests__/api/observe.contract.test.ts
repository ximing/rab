 ...
 * 避免全局配置跨文件污染。
 */
import { observe, observable, configure, resetGlobalConfig } from '../../main';
import type { Reaction, ReactionScheduler, Operation } from '../../main';
...
describe('observe() 公开行为契约', () => {
  ...
  describe('返回值与包装', () => {
    test('observe(fn) 返回 reaction 函数，调用它透传 fn 的返回值', () => {
      ...
      const reaction = observe(() => state.x * 10, { lazy: true });
