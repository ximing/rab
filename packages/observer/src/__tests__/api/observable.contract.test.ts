...
describe("observable() 公开行为契约", () => {
  ...
  describe("不可包装对象原样返回", () => {
    ...
    test("跨 realm 普通对象仍被正常包装且深层响应式", () => {
      const crossRealmObj = vm.runInNewContext(
        "({ nested: { value: 1 } })"
    ...
    test("跨 realm Map 仍路由到集合插桩，方法读写响应式", () => {
      const crossRealmMap = vm.runInNewContext(
        "new Map([['a', 1]])"
  ...
  describe("安全边界", () => {
    ...
    test("原型链上的 constructor 读取保持原生语义，不被包装", () => {
      const state = observable({ count: 0 });
      expect((state as unknown as Record<string, unknown>).constructor).toBe(
        Object
      );
      expect(
        isObservable((state as unknown as Record<string, unknown>).constructor)
      ).toBe(false);
      ...
      expect(
        (thing as unknown as Record<string, unknown>).constructor
      ).toBe(Thing);
    });
  });
  ...
  describe("options 语义（per-raw 键控 / first-wins / partial 回落默认）", () => {
    ...
    test("first-wins：对同一 raw 第二次传不同 options 被静默忽略（返回同一代理，首个 options 继续生效）", () => {
      ...
      const first = jest.fn(
        (_t: object, _k: PropertyKey, reactions: Reaction[]) => reactions
      );
