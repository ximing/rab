...
describe("契约: WeakMap / WeakSet 可追踪但不可枚举", () => {
  ...
  test("WeakMap/WeakSet 保持 Weak 语义：无法枚举内容、非对象 key 抛 TypeError（文档级断言）", () => {
    ...
    expect(() => (wm as unknown as Map<string, number>).set("str", 1)).toThrow(
      TypeError
    );
...
describe("契约: ES2024 Set 方法（union/intersection/difference 等）", () => {
  ...
  const withMethods = <T>(s: Set<T>): SetWithES2024<T> =>
    s as unknown as SetWithES2024<T>;
  ...
  test("deep 模式下 ES2024 Set 方法可正常调用且结果正确", () => {
    ...
    expect(new Set(set.intersection(new Set([2, 3, 4])))).toEqual(
      new Set([2, 3])
    );
