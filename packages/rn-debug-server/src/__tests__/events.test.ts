...
describe('events bus /events', () => {
  ...
  it('设备上线/下线广播 connected/disconnected', async () => {
    ...
    await waitFor(
      () =>
        events.some(
          e =>
            (e as { kind: string }).kind === 'device' &&
            (e as { action: string }).action === 'connected'
        ),
    ...
    await waitFor(
      () =>
        events.some(
          e =>
            (e as { kind: string }).kind === 'device' &&
            (e as { action: string }).action === 'disconnected'
        ),
  ...
  it('指令 sent/completed 事件广播', async () => {
    ...
    await waitFor(
      () => devReceived.some(m => messageKind(m) === 'command'),
      'command delivered for events test'
    );
    ...
    await waitFor(
      () =>
        events.some(
          e =>
            (e as { kind: string }).kind === 'command' &&
            (e as { action: string }).action === 'sent'
        ) &&
        events.some(
          e =>
            (e as { kind: string }).kind === 'command' &&
            (e as { action: string }).action === 'completed'
        ),
